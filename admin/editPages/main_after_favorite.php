...
if(isset($_POST['offerDone']) && !empty($_POST['offerTitle'])){
    ...
    $tit = $db->query("UPDATE `after_favorite_page` SET `title` = '".$title."' WHERE `id` = 1;");
    $con = $db->query("UPDATE `after_favorite_page` SET `content` = '".$content."' WHERE `id` = 1;");
...
?>
<h2>Редактироуем тест после фаворитов</h2>
    <div class="content">
    <?
